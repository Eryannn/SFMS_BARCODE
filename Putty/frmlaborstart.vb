...
Public Class frmlaborstart
    ...
    Private Sub frmlaborstart_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub txtopernum_TextChanged(sender As Object, e As EventArgs) Handles txtopernum.TextChanged
        ...
        Dim cmdcheckpending As SqlCommand = New SqlCommand("Select * from sfms_jobtran where job = @job AND Suffix = @suffix AND oper_num = @opernum AND end_time IS NULL AND trans_type='R' AND emp_num=@empnum ", con)
        ...
        Try
            ...
            If readsfms.HasRows Then
                frmlaborend.txtjob.Text = txtjob.Text
                frmlaborend.txtsuffix.Text = txtsuffix.Text
                MsgBox("Setup is still on process!")
                frmlaborend.Show()
                frmlaborend.lblempnum.Text = lblempnum.Text
                frmlaborend.txtopernum.Text = txtopernum.Text
                frmlaborend.lblempname.Text = lblempname.Text
                Me.Close()
            Else
                readsfms.Close()
